...
void loop() {
  ...
  //recieving stop command
    ...
    if(velocity_PWM < 0){
        SetSpeedPIDD('B', abs(velocity_PWM), '-');
    } 
    else SetSpeedPIDD('B',abs(velocity_PWM), '+');

    if(distance <= 10){
      stopMotors();
    }
    /*if((median_speed == 0) && (distance > 25)){
        median_speed = 120;
        velocity_PWM = 0;
        proportional = 0;
        integral = 0;
      ...
      Serial.println(median_speed);
  }
  else if (cmd == 'u'){
    PIDcalculatev();

    if(velocity_PWMv < 0){
        SetSpeedPIDv('B', abs(velocity_PWMv), '-');
    } 
    else SetSpeedPIDv('B',abs(velocity_PWMv), '+');

  client.flush();
  client.print("Current RPM" + String(actual_RPM) + " Target RPM: " + String(target_RPM));
  delay(100);
